...
public class PercentPieView extends View {
    ...
    private void drawData(Canvas canvas, float degree, int i, float percent) {
        ...
        //获取名称文本大小
        dataPaint.getTextBounds(names.get(i), 0, names.get(i).length(), dataTextBound);
        //绘制名称数据，20为纵坐标偏移量
        canvas.drawText(names.get(i),
     ...
     * @param names   名称数组
     */
    public void setData(List<Integer> numbers, List<String> names) {
        if (numbers == null || numbers.size() == 0
     ...
     * @param colors  颜色数组
     */
    public void setData(List<Integer> numbers, List<String> names, int[] colors) {
        if (numbers == null || numbers.size() == 0
                || names == null || names.size() == 0
                || colors == null || colors.length == 0) {
            return;
        }
        if (numbers.size() != names.size() || numbers.size() != colors.length) {
            return;
        }
        this.numbers = numbers;
        this.names = names;
