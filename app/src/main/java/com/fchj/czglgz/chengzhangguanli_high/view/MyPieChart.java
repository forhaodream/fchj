...
public class MyPieChart extends View {
    ...
    private Paint paint, mBlankPaint;  //画笔
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < pieEntries.size(); i++) {
            ...
            canvas.drawArc(rectF, (float) startC, (float) sweep, true, paint);

            //画分割线
            canvas.drawArc(rectF, (float) startC, 1, true, mBlankPaint);
            if ((pieEntries.get(i).getNumber() > 0 && total > 0) || (total <= 0 && pieEntries.get(i).getNumber() <= 0)) {
                //下面是画扇形外围的 短线和百分数值。
                double arcCenterC = startC + sweep / 2; //当前扇形弧线的中间点和圆心的连线 与 起始角度的夹角
    ...
    public static class PieEntry {
        ...
        private double startC;     //对应扇形起始角度
        private double endC;       //对应扇形结束角度

        public PieEntry(double number, int colorRes, boolean selected) {
        ...
        public void setEndC(double endC) {
            this.endC = endC;
        }
