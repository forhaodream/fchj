...
import com.fchj.czglgz.chengzhangguanli_high.model.SchoolNewsDetailModel;
...
public class SchoolDetailActivity extends Activity {
    ...
    private int newsId;
    private SchoolNewsDetailModel mSchoolNewsDetailModel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mJSObject = new JSObject(SchoolDetailActivity.this);
    ...
    private void setZiXun() {
        ...
        builder.add("cId", String.valueOf(newsId));
        Request request = new Request.Builder().url(Url.schoolDetialUrl).post(builder.build()).build();
            ...
            public void onResponse(Response response) {
                try {
                    if (response.isSuccessful()) {
                        ...
                        mSchoolNewsDetailModel = gson.fromJson(reader, SchoolNewsDetailModel.class);
                        if (mSchoolNewsDetailModel.getCode() == 1) {

                            mHandler.post(new Runnable() {
                                @Override
                                public void run() {
                                    url = mSchoolNewsDetailModel.getData().getNewscontenttexturl();
                                    addWebView();
                                }
                            });
                        } else if (mSchoolNewsDetailModel.getCode() == 2) {
                            Looper.prepare();
                            Toast.makeText(SchoolDetailActivity.this, mSchoolNewsDetailModel.getMsg() + "", Toast.LENGTH_SHORT).show();
                            Looper.loop();
                        } else {
                            Looper.prepare();
                            Toast.makeText(SchoolDetailActivity.this, mSchoolNewsDetailModel.getMsg() + "", Toast.LENGTH_SHORT).show();
                        ...
                        Looper.prepare();
                        Toast.makeText(SchoolDetailActivity.this, "服务器异常", Toast.LENGTH_SHORT).show(); //请求失败
                        Looper.loop();
                    }
                } catch (IOException e) {
                    e.printStackTrace();
