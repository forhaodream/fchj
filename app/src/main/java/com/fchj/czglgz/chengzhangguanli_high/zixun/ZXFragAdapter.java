...
public class ZXFragAdapter extends BaseAdapter {
    private List<ZXFragModel.DataBean> mData;

    public ZXFragAdapter(List<ZXFragModel.DataBean> data) {
        this.mData = data;
    }
...
//        holder.contentTv.setText(mData.get(position).getContenttext());
        Glide.with(holder.img.getContext()).load(Url.fileUrl + mData.get(position).getPreviewimageurl()).transform(new GlideRoundTransform(holder.img.getContext(), 8)).placeholder(R.mipmap.zhongguo).error(R.mipmap.zhongguo).into(holder.img);
