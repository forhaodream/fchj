...
public class ZiXunDetailActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mJSObject = new JSObject(ZiXunDetailActivity.this);
    ...
    private void setZiXun() {
        ...
        builder.add("iId", String.valueOf(newsId));
        Request request = new Request.Builder().url(Url.zxDetailUrl).post(builder.build()).build();
            ...
            public void onResponse(Response response) {
                try {
                    if (response.isSuccessful()) {
                        ...
                        mZiXunDetailModel = gson.fromJson(reader, ZiXunDetailModel.class);
                        if (mZiXunDetailModel.getCode() == 1) {

                            mHandler.post(new Runnable() {
                                @Override
                                public void run() {
                                    url = mZiXunDetailModel.getData().getContenttexturl();
                                    addWebView();
                                }
                            });
                        } else if (mZiXunDetailModel.getCode() == 2) {
                            Looper.prepare();
                            Toast.makeText(ZiXunDetailActivity.this, mZiXunDetailModel.getMsg() + "", Toast.LENGTH_SHORT).show();
                            Looper.loop();
                        } else {
                            Looper.prepare();
                            Toast.makeText(ZiXunDetailActivity.this, mZiXunDetailModel.getMsg() + "", Toast.LENGTH_SHORT).show();
                        ...
                        Looper.prepare();
                        Toast.makeText(ZiXunDetailActivity.this, "服务器异常", Toast.LENGTH_SHORT).show(); //请求失败
                        Looper.loop();
                    }
                } catch (IOException e) {
                    e.printStackTrace();
